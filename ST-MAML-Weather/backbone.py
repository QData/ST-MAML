...
# class Linear_fw_np(nn.Linear): #used in MAML to forward input with fast weight
#     def __init__(self, in_features, out_features):
#         super(Linear_fw, self).__init__(in_features, out_features)
#         self.weight.fast = None #Lazy hack to add fast weight link
#         self.bias.fast = None
...
class Regressor(nn.Module):
    def __init__(self, input_dim=16, aug_dim=20, hidden_dim=[100, 100], output_dim=1):
        super(Regressor, self).__init__()
        self.output_dim = output_dim
        self.input_dim = input_dim
        self.aug_dim = aug_dim
        self.hidden_dim = hidden_dim
        self.learner = nn.Sequential(Linear_fw(input_dim+aug_dim, hidden_dim[0]), nn.ReLU(), 
            Linear_fw(hidden_dim[0], hidden_dim[1]), nn.ReLU())
    def forward(self, x):
        out = self.learner(x)
        return out

class RegressorMAML(nn.Module):
    def __init__(self, input_dim=16, hidden_dim=[100, 100], output_dim=1):
        super(RegressorMAML, self).__init__()
        self.output_dim = output_dim
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.aug_dim=0
        self.learner = nn.Sequential(Linear_fw(input_dim, hidden_dim[0]), nn.ReLU(), 
            Linear_fw(hidden_dim[0], hidden_dim[1]), nn.ReLU())
    def forward(self, x):
        out = self.learner(x)
        return out
