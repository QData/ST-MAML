    ...
    if mode == 'train':
        ...
        for task_train, task_val in dataset:
            ...
            for train, val in zip(task_train, task_val):
                ...
                fast_weights = OrderedDict((name, param) for (name, param) in model.learner.named_parameters())
                for i in range(args.inner_step):
                    if i == 0:
                        grads = torch.autograd.grad(loss, model.learner.parameters(), create_graph=True)
...
def meta_eval_one_epoch(model, dataset, epoch, args, mode='eval', device='cuda', print_freq=100):
    if mode == 'eval':
        model.eval()
        epoch_loss = 0.
        batch_idx = 0
        ...
        f.close()
        print(print_str)
        return print_str, epoch_rec_loss/batch_idx


def meta_test_one_epoch(model, dataset, args, mode='test', device='cuda', print_freq=100):
    if mode == 'test':
        model.eval()
        epoch_loss = 0.
        batch_idx = 0
        ...
        print_str = 'Test | Epoch Avg Loss: {:.3f}, Epoch Avg Rec Loss: {:.3f}'.format(
                                                                                        epoch_loss / batch_idx,
                                                                                        epoch_rec_loss / batch_idx)
        print(print_str)
        return print_str, epoch_rec_loss/batch_idx
