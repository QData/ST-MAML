...
def linear_block(iput, aug, param, prefix):
    ...
    out = F.linear(out, weight=param[prefix+'linear5.weight'])
    return out
...
class LR(nn.Module):
    def __init__(self, n_input, n_h, n_output, n_out_aug):
        ...
        self.mlp.add_module('linear1', nn.Linear(n_input+n_out_aug, n_h[0]))
        self.mlp.add_module('relu1', nn.ReLU())
        self.mlp.add_module('linear2', nn.Linear(n_h[0], n_h[1]))
        ...
        self.mlp.add_module('linear5', nn.Linear(n_h[3], n_output, bias=False))
...
class ParamEnc(nn.Module):
    def __init__(self, n_xyaug, n_hid, n_out):
        ...
        self.param_encoder.add_module('xy2hid', nn.Linear(n_xyaug, n_hid))
        self.param_encoder.add_module('relu1', nn.ReLU(inplace=True))
        self.param_encoder.add_module('hid2out', nn.Linear(n_hid, n_out))
...
class TaskEnc(nn.Module):
    def __init__(self, n_in, n_hid, n_out):
        ...
        self.task_enc.add_module('relu1', nn.ReLU())
        self.task_enc.add_module('hid2hid', nn.Linear(n_hid[0], n_hid[1]))
        self.task_enc.add_module('relu2', nn.ReLU())
        self.task_enc.add_module('hid2out', nn.Linear(n_hid[1], n_out))
        # self.task_enc.add_module('relu3', nn.ReLU())
        # self.task_enc.add_module('hid2out', nn.Linear(n_hid[2], n_out))
...
class EncModel(nn.Module):
    def __init__(self, n_in, n_hid, n_out, n_hid_emb, n_out_emb, n_xy, n_hid_aug, n_out_aug, aug_enc=False, model_type='prob', act_type='softplus'):
    ...
    def task_encoder(self, x_spt, y_spt, x_qry, y_qry=None):
        if self.model_type=='prob':
            ...
            if y_qry is not None:
                ...
                post_deter_task_code = self.deter_taskenc(x_qry, y_qry)
